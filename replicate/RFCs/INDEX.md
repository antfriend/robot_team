...
- [TTDB-RFC-0010-Stigmergic-Fields-and-Record-Identity.md](TTDB-RFC-0010-Stigmergic-Fields-and-Record-Identity.md): Stigmergic Fields, Lane Discipline, and Stable Record Identity — defines the two memory disciplines a TTDB may hold (**EVIDENCE** append-only, **FIELD** decaying-and-reinforced, **PROVENANCE** never reclaimed) and requires every lane to declare which it is, with EVIDENCE as the fail-safe default. Registers the fleet's complete lane allocation (§3: `lat<90` + 91–97 evidence, 90/98/99/100 provenance, **no FIELD lane exists yet**; the first must be new at `@LAT101+`). Fences decay away from anything that must not silently rewrite itself (§6: no measured constant from a field, no `derived_from` into one, and the acceptance test that the system stays correct with the field empty). Retires ordinal record names via TTDB-RFC-0004 §2's already-sanctioned stable hash — `sid:<8 hex>` as identity with the coordinate kept as address, citations optionally carrying `#sid` and resolving `stale` on mismatch, both forms permanently live. Decay is evaluated on read and never written. (Draft; **nothing implemented** — §3 classifies what exists, §§4–6 describe a mechanism no node has run; companion design doc `stigmergy.md`; the divergence it retires is recorded as belief `@LAT98LON6`; builds on TTDB-RFC-0001/-0003/-0004/-0005, relates to TTN-RFC-0011)
